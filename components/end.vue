<template>
    <main>

        <div>

        <footer class="bg-primary text-center text-lg-start text-white">

            <div class="container p-4">

            <div class="row mt-4">

                <div class="col-lg-3 col-md-6 mb-4 mb-md-0">
                <h5 class="text-uppercase"><p class="text-left">What Is A MA'RIFAT ?</p></h5>

                <ul class="list-unstyled mb-0">
                    <li>
                    <a style="text-decoration: none;" class="text-white"><i class="  fa-fw fa-sm me-2"></i><p class="text-justify">Ma'rifat History</p></a>
                    </li>
                    <li>
                    <a style="text-decoration: none;" class="text-white"><i class="  fa-fw fa-sm me-2"></i><p class="text-justify">Organizational structure</p></a>
                    </li>
                    <li>
                    <a style="text-decoration: none;" class="text-white"><i class="  fa-fw fa-sm me-2"></i><p class="text-justify">Activities and Events</p></a>
                    </li>
                </ul>
                </div>

                <div class="col-lg-3 col-md-6 mb-4 mb-md-0">
                <h5 class="text-uppercase"><p class="text-left">ROHIS MA'RIFAT SMKN 4</p></h5>

                <ul class="list-unstyled">
                    <li>
                    <NuxtLink class="card__link" to="/irma"></NuxtLink><a style="text-decoration: none;" class="text-white"><i class=" fa-fw fa-sm me-2"></i><p class="text-justify">Rohis History</p></a>
                    </li>
                    <li>
                    <NuxtLink class="card__link" to="/irma"></NuxtLink><a style="text-decoration: none;" class="text-white"><i class=" fa-fw fa-sm me-2"></i><p class="text-justify">Organizational structure</p></a>
                    </li>
                    <li>
                    <NuxtLink class="card__link" to="/irma"></NuxtLink><a style="text-decoration: none;" class="text-white"><i class=" fa-fw fa-sm me-2"></i><p class="text-justify">Activities and Events</p></a>
                    </li>
                </ul>
                </div>



                <div class="col-lg-3 col-md-6 mb-4 mb-md-0">
                <h5 class="text-uppercase"><p class="text-left">IRMA MA'RIFAT SMKN 4</p></h5>

                <ul class="list-unstyled">
                    <li>
                    <p class="text-justify">Irma's History</p>
                    </li>
                    <li>
                    <p class="text-justify">Organizational structure</p>
                    </li>
                    <li>
                    <p class="text-justify">activities and events</p>
                    </li>
                </ul>
                </div>



                <div class="col-lg-3 col-md-6 mb-4 mb-md-0">
                <h5 class="text-uppercase"><p class="text-left">Account Official MA'RIFAT</p></h5>

                <ul class="list-unstyled">
                    <li>
                    <a style="text-decoration: none;" class="text-white"><i class=" fa-fw fa-sm me-2"></i><p class="text-justify">Instagram</p> </a>
                    </li>
                    <li>
                    <a style="text-decoration: none;" class="text-white"><i class=" fa-fw fa-sm me-2"></i><p class="text-justify">Facebook</p></a>
                    </li>
                    <li>
                    <a style="text-decoration: none;" class="text-white"><i class=" fa-fw fa-sm me-2"></i><p class="text-justify">Whatsapp</p></a>
                    </li>
                </ul>
                </div>

            </div>

            </div>

            <div class="text-center p-3" style="background-color: #1E90FF">
            © 2022 Copyright
            <a class="text-white" style="text-decoration: none;" href="https://majelisremajaislam04.netlify.app">MA'RIFAT SMKN 4 Tasikmalaya</a>
            </div>

        </footer>
        </div>
    </main>
</template>
